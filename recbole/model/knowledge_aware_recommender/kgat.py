...
class KGAT(KnowledgeRecommender):
    ...
    def _get_ego_embeddings(self):
        ...
        imageEmbeddingPath = 'path_to_image_embeddings.csv'
        descriptionEmbeddingPath = 'path_to_text_embeddings.csv'
        imageEmbedding = np.genfromtxt(imageEmbeddingPath, dtype=np.float32)
        textEmbedding = np.genfromtxt(descriptionEmbeddingPath, dtype=np.float32)
        imageTensor = torch.from_numpy(imageEmbedding)
        textTensor = torch.from_numpy(textEmbedding)
        catTensor = torch.cat([imageTensor, textTensor], dim=1)

        # Combine image and text embeddings with a linear layer
        combined_embeddings = torch.nn.Linear(catTensor.shape[1], self.embedding_size)(catTensor)

        ego_embeddings = torch.cat([user_embeddings, combined_embeddings], dim=0)
        return ego_embeddings

    def generate_mm_transE_score(self, hs, ts, r):
        all_embeddings = self._get_mm_embeddings()
